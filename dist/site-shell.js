...
(function(){"use strict";var a=typeof document<"u"?document.currentScript:null;let c="";function v(){if(c)return;if(document.currentScript?.src){c=document.currentScript.src.replace(/[^/]+$/,"");return}try{if(a&&a.tagName.toUpperCase()==="SCRIPT"&&a.src||new URL("site-shell.js",document.baseURI).href){c=(a&&a.tagName.toUpperCase()==="SCRIPT"&&a.src||new URL("site-shell.js",document.baseURI).href).replace(/[^/]+$/,"");return}}catch{}const e=document.querySelector('script[src*="site-shell"]');if(e?.src){c=e.src.replace(/[^/]+$/,"");return}console.warn("[site-shell] Could not detect script base path")}function h(){return c}async function m(e){let t;try{t=await fetch(e)}catch(s){return console.error(`[site-shell] Network error loading config from ${e}:`,s),u()}if(!t.ok)return console.error(`[site-shell] Config load failed: ${t.status} from ${e}`),u();let r;try{r=await t.json()}catch(s){return console.error(`[site-shell] Invalid JSON in config from ${e}:`,s),u()}return w(r)}function w(e){return{...e,tissue:e.tissue||"",menu:Array.isArray(e.menu)?e.menu:[],cfde_logo:e.cfde_logo||"",tissue_logo:e.tissue_logo||"",cfde_wheel:e.cfde_wheel||"",nih_logo:e.nih_logo||"",drc_logo:e.drc_logo||"",kc_logo:e.kc_logo||"",footer:e.footer||""}}function u(){return{tissue:"",menu:[],cfde_logo:"",tissue_logo:"",cfde_wheel:"",nih_logo:"",drc_logo:"",kc_logo:"",footer:""}}function l(e){if(!e||typeof e!="string"||/^(javascript|data):/i.test(e.trim()))return"";if(/^(https?:)?\/\//.test(e)||e.startsWith("/"))return e;const t=h();return new URL("../public/",t).href+e}function i(e){if(typeof e!="string")return"";const t=e.trim();if(!t)return"";const r=t.replace(/[\u0000-\u001F\u007F\s]+/g,"").toLowerCase();return/^(javascript|data|vbscript):/.test(r)?"":t}function n(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function d(e,t){if(!e)return!1;const r=e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");return new RegExp(`${r}($|[?#/&])`).test(t)}function b(e){const t=window.location.href;return`
    <div class="main-menu">
        ...
        <div class="menu-item-wrapper ${d(s.path,t)?"active":""}">
            ${s.path?`<a class="menu-item" href="${n(i(s.path))}">${n(s.label)}</a>`:`<div class="menu-item">${n(s.label)}</div>`}
            ${o}
        </div>
        `}).join("")}
    </div>
    `}function C(e){return setTimeout(()=>{f(),$()},0),`
    <header class="site-header">
        <div class="nav">
            <div class="nav-left">
                <img src="${n(i(l(e.cfde_logo)))}"/>
                <img src="${n(i(l(e.tissue_logo)))}"/>
            </div>
            <a class="nav-title" href="/">
                <!--<img src="images/title.png" />-->
                <div class="title-group">
                    <div class="title-cfde">CFDE</div>
                    <div class="title-name">${n(e.tissue)}<br/>Resource</div>
                </div>
            </a>
            <div class="nav-menu">
            ...
            <div class="nav-right">
                <img src="${n(i(l(e.cfde_wheel)))}" />
            </div>
        </div>
    </header>
    `}function $(){window.addEventListener("popstate",f)}function f(){const e=window.location.href;document.querySelectorAll(".submenu-item").forEach(t=>{const r=t.getAttribute("href"),s=d(r,e);t.classList.toggle("active",s)}),document.querySelectorAll(".menu-item-wrapper").forEach(t=>{const r=t.getAttribute("href"),s=d(r,e);t.classList.toggle("active",s)})}function _(e){return`
    <div class="footer-menu">
        ${e.map(r=>{const s=r.submenu?.length?`
            ${r.submenu.map(o=>`
              <a class="" href="${n(i(o.path))}">
                  ${n(o.label)}
              </a>
            `).join("")}
            `:"";return`
        <div style="display:flex; flex-direction:column; gap:3px">
            ${r.path?`<a style="font-weight:bold" href="${n(i(r.path))}">${n(r.label)}</a>`:`<a style="font-weight:bold">${n(r.label)}</a>`}
            ${s}
        </div>
        `}).join("")}
    </div>
      ...
      <div class="footer-inset">
          <div style="background: white; height:50px">
              <a href="https://data.cfde.cloud/" target="_blank">
                <img src="${n(i(l(e.drc_logo)))}"  style="height:50px"/>
              </a>
          </div>
          ...
          <div class="f-row align-v-center" style="gap:10px">
              <a href="https://cfdeknowledge.org/" target="_blank">
                <img src="${n(i(l(e.kc_logo)))}" style="height:50px"/>
              </a>
          </div>
      </div>
  ...
  `}async function g(){const e=document.getElementById("site-header"),t=document.getElementById("site-footer");if(!e&&!t){console.warn("[site-shell] No #site-header or #site-footer found");return}try{const r=e?.getAttribute("data-config-url")||l("config/site-config.json"),s=await m(r);if(e){e.innerHTML=C(s);const o=e.parentElement;o&&!o.classList.contains("cfde__site-shell")&&o.classList.add("cfde__site-shell")}t&&(t.innerHTML=y(s))}catch(r){console.error("[site-shell] Failed to initialize:",r)}}v(),document.readyState!=="loading"?g():document.addEventListener("DOMContentLoaded",g)})();
