...
function createSubmenu(submenuItems, currentUrl) {
    ...
    for (const sub of submenuItems) {
        ...
        const safePath = sanitizeUrl(sub.path);
        if (isActive(safePath, currentUrl)) {
            link.classList.add("site-shell__submenu-item--active");
        }
        if (safePath) link.href = safePath;
    ...
    for (const item of menu) {
        ...
        if (item.submenu?.length) {
            ...
            for (const sub of item.submenu) {
                if (isActive(sanitizeUrl(sub.path), currentUrl)) {
...
function createSearchIcon() {
    ...
    path.setAttribute(
        "d",
        "M15.7955 15.8111L21 21M18 10.5C18 14.6421 14.6421 18 10.5 18C6.35786 18 3 14.6421 3 10.5C3 6.35786 6.35786 3 10.5 3C14.6421 3 18 6.35786 18 10.5Z",
    );
...
function updateActiveMenu() {
    ...
    document.querySelectorAll(".site-shell__submenu-item").forEach((link) => {
        ...
        link.classList.toggle(
            "site-shell__submenu-item--active",
            isActive(href, currentUrl),
        );
    });
    document
        .querySelectorAll(".site-shell__menu-item-wrapper")
        .forEach((wrapper) => {
            const link = wrapper.querySelector(".site-shell__menu-item");
            const href = link?.getAttribute("href");
            const hasActiveSub = wrapper.querySelector(
                ".site-shell__submenu-item--active",
            );
